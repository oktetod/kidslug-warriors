...
import { Scene as BabylonScene, FreeCamera, Vector3, Color4 } from '@babylonjs/core';
...
function ensureMenuScene(): ScreenManager {
  if (!menuBabylonScene || !screenMgr) {
    ...
    menuBabylonScene.clearColor = new Color4(0.02, 0.02, 0.08, 1);

    // Babylon.js requires an active camera to render anything, including GUI
    const cam = new FreeCamera('menuCam', new Vector3(0, 0, -1), menuBabylonScene);
    cam.setTarget(Vector3.Zero());
    menuBabylonScene.activeCamera = cam;
...
// Track whether a login attempt is in progress (prevents double showAuth)
let loginInProgress = false;

function showAuth(errMsg?: string) {
  if (loginInProgress) return; // do not rebuild screen during active login
      ...
      loginWithGoogle()
        .then(r => {
          ...
          if (r.user) {
            ...
            // Popup closed by user = not a real error, just stay on screen
            const isUserCancelled =
              !r.error ||
              r.error.includes('popup-closed-by-user') ||
              r.error.includes('popup-blocked') ||
              r.error.includes('cancelled-popup-request');
            if (!isUserCancelled) showAuth(r.error ?? undefined);
            // else: do nothing - screen is already showing, user just closed popup
          }
        })
        .catch((e: unknown) => {
          loginInProgress = false;
...
async function bootApp() {
  ...
  try {
    onAuthChange(async (user) => {
      // If user just dismissed a login popup, Firebase fires null here too.
      // Ignore this null signal while a login attempt is still in progress.
      if (loginInProgress) return;
